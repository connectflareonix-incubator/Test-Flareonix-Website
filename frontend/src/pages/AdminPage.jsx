...
const AdminPage = () => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [d, u, r, m, c] = await Promise.all([
      ...
      setData({ stats: d.data.stats, users: u.data, reviews: r.data, messages: m.data, cases: c.data });
  ...
  const delReview = async (id) => {
    ...
    try { await axios.delete(`${API}/reviews/${id}`, getAuth()); toast.success('Deleted'); load(); } catch { toast.error('Failed'); }
  };
  ...
  const exp = async (t) => {
    try {
      ...
      const a = document.createElement('a'); a.href = URL.createObjectURL(b); a.download = `${t}.json`; a.click();
    } catch { toast.error('Failed'); }
  };
      ...
      <div className="min-h-screen bg-[#050505] flex items-center justify-center p-6">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={login} className="p-6 glass rounded-xl border border-white/10 space-y-4">
            <Input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="Email" className="bg-black/50 border-white/10 text-white" required />
            <Input type="password" value={pwd} onChange={e => setPwd(e.target.value)} placeholder="Password" className="bg-black/50 border-white/10 text-white" required />
            <Button type="submit" disabled={loading} className="w-full bg-primary">{loading ? 'Loading...' : 'Login'}</Button>
          </form>
    ...
    <div className="min-h-screen bg-[#050505]">
      ...
      <div className="max-w-6xl mx-auto p-6">
        <div className="flex gap-2 mb-6 flex-wrap">
          {['overview', 'users', 'reviews', 'messages'].map(t => (
            <button key={t} onClick={() => setTab(t)} className={`px-4 py-2 rounded-full text-sm ${tab === t ? 'bg-primary text-white' : 'bg-white/5 text-muted-foreground'}`}>{t}</button>
          ))}
        </div>
