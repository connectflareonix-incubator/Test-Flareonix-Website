...
import { Calendar, MapPin, Flame, ArrowRight, MessageSquare } from 'lucide-react';
            ...
            <div className="grid md:grid-cols-2 gap-6" data-stagger-children>
              {events.map((ev) => (
                <Link
                  key={ev.id}
                  to={`/events/${ev.id}`}
                  className="group block p-6 rounded-2xl bg-[#0f0f0f] border border-white/10 hover:border-primary/40 transition-all hover:-translate-y-1"
